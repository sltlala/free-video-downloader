...
async def summarize_video(req: SummarizeRequest, user: dict | None = Depends(get_optional_user)) -> AsyncIterable[
    ServerSentEvent]:
    """
    AI 视频总结（SSE 流式）
    事件类型: subtitle / summary / mindmap / done
    """

    yield ServerSentEvent(
        raw_data=json.dumps({"need_login": user is None}, ensure_ascii=False),
        event="error",
    )
...
async def chat_with_video(req: ChatRequest, user: dict | None = Depends(get_optional_user)) -> AsyncIterable[
    ServerSentEvent]:
    """AI 视频问答（SSE 流式）"""
    try:
